...
//! per-source mutex used by `serve` is process-local, so the CLI does
//! not coordinate with a running `serve`'s cron polls — two processes
//! could resolve the same source at once (redundant work, not
//! corruption, as long as the DB lives on a real filesystem rather than
//! a macOS bind mount). For coordinated catch-up while `serve` is up,
//! prefer the `POST /sources/{name}/backfill` endpoint, which runs
...
pub async fn run(config_path: PathBuf, source_name: String, pages: u32) -> Result<()> {
    ...
    let totals = backfill_source::run(
        source,
        db,
        metadata,
        cfg.ingestion.clone(),
